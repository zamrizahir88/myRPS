-- ============================================================================
--  myRPS — STAFF PROFILE, RPS CARD, AND OPT-IN STUDENT PROFILES
--
--  * gives the RPS their own profile fields instead of a student form
--  * adds the card students use to reach you (WhatsApp, office, CV link)
--  * lets a student choose to show classmates their photo, semester,
--    psychometric persona and pillar count — all off by default
--
--  Run it ONCE, after MIGRATE_AVATARS.sql.
...
-- ============================================================================
select 'staff fields' as item,
       case when (select count(*) from information_schema.columns
                   where table_schema='public' and table_name='profiles'
                     and column_name in ('title','position_title','office_location','cv_url','whatsapp')) = 5
            then 'OK' else 'FAILED' end as result
union all
select 'RPS card',
       case when exists (select 1 from information_schema.views
                          where table_schema='public' and table_name='rps_card')
            then 'OK' else 'FAILED' end
union all
select 'student sharing is off by default',
       case when not exists (select 1 from public.profiles where share_profile)
            then 'OK' else 'someone is already sharing' end;
