-- ============================================================================
--  myRPS — EXEMPTED SUBJECTS, AND THE CREDIT SPLIT
--
--  Exempted subjects move out of the semesters and into their own section:
--  an exemption did not happen in a semester, and filing it under one made
--  that semester report subjects the student never took.
--
--  It also splits the credit totals into three: credits earned by sitting the
--  subject, credits granted as exemptions, and the load being carried in the
--  semester the student is in now.
--
--  Existing exempted records are moved for you. Nothing else is touched.
--
--  Run it ONCE, after MIGRATE_DEMO_STUDENT.sql.
...
-- ============================================================================
--  Result — all four rows should say OK.
-- ============================================================================
select 'exemptions sit outside the semesters' as item,
       case when not exists (select 1 from public.student_records
                              where state = 'exempted' and term_id is not null)
            then 'OK' else 'FAILED' end as result
union all
select 'one exemption per subject',
       case when to_regclass('public.student_records_one_exemption') is not null
            then 'OK' else 'FAILED' end
union all
select 'a record with no semester must be an exemption',
       case when exists (select 1 from pg_constraint
                          where conname = 'student_records_termless_is_exemption')
            then 'OK' else 'FAILED' end
union all
select 'credits split into taken and exempted',
